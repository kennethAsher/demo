...
path = glob.glob(f'{get_desktop_path()}/data/*.xls*')[0]
...
for i in range(30):
    # 不断去拷贝第一页并重命名
    sheet = workbook.copy_worksheer(sheet_init)
    sheet.title = f'{i+1}日'
    for j in range(random.randint(10, 30)):
        for row in sheet.iter_rows(min_row=3+j, max_row=3+j):
            ...
            # 嵌套循环， 对当前行的格子进行遍历吧内容写入
            for index, k in enumerate(info):
                row[index].value = k
    print(f'第{i+1}日已完成')
workbook.save(filename=f'{get_desktop_path()}/data/results.xlsx')
